use {
    ...
    solana_accounts_db::accounts_index::{IndexKey, ScanConfig, ScanError},
...
pub fn update_v1(
    ...
    let accumulator_data = {
        ...
        let accounts_data = &mut borsh::to_vec(&messages)?;
        data.append(acc_state_magic);
        data.append(&mut borsh::to_vec(&bank.slot())?);
        data.append(&mut borsh::to_vec(&ACCUMULATOR_RING_SIZE)?);
...
fn post_accumulator_attestation(
    bank: &Bank,
    acc: pythnet_sdk::accumulators::merkle::MerkleTree<
    ...
    let mut sequence: AccumulatorSequenceTracker = {
        let data = bank
            ...
            .unwrap_or_default();
        let data = data.data();
        solana_sdk::borsh1::try_from_slice_unchecked(data)
    ...
    sequence.sequence += 1;
    let sequence = borsh::to_vec(&sequence)
    ...
    let sequence_account = {
        ...
        account.set_data_from_slice(&sequence);
        account
    };
    ...
    let message_account = {
        ...
        account.set_data_from_slice(&message);
        account
    };
...
pub fn update_v2(bank: &Bank) -> std::result::Result<(), AccumulatorUpdateErrorV1> {
    ...
    for (pubkey, mut account) in accounts {
        ...
        match pyth_oracle::validator::aggregate_price(
            ...
            Ok(messages) => {
                account.set_data_from_slice(&price_account_data);
