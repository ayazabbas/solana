...
fn test_update_accumulator_sysvar() {
    ...
    let _ = PriceAccount::initialize(
        &AccountInfo::new(
            ...
            true,
            &mut 0,
            price_feed_account.data_as_mut_slice(),
    ...
    assert_eq!(
        ...
        borsh::to_vec(&expected_wormhole_message).unwrap()
    );
...
fn new_from_parent(parent: Arc<Bank>) -> Bank {
    Bank::new_from_parent(parent.clone(), &Pubkey::default(), parent.slot() + 1)
}
...
fn test_accumulator_v2(generate_buffers: [bool; 4]) {
    ...
    let generate_price = |seeds, generate_buffers: bool| {
        ...
        let messages = {
            ...
            let price_feed_info_data = price_feed_account.data_as_mut_slice();
